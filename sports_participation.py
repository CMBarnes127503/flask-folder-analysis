...
# ---------------------------------------------------------
# ROUTES — UPDATED TO USE index.html
# ---------------------------------------------------------

# Sports home — index.html handles everything
@sports_bp.route("/sports", methods=["GET", "POST"])
def sports_home():
    return render_template("index.html")


# Upload dataset — index.html form must point here
@sports_bp.route("/sports/upload", methods=["POST"])
def sports_upload():
    global sports_df

    file = request.files.get("sports_file")
    if not file:
        return render_template("index.html", message="No sports file uploaded")

    sports_df = pd.read_excel(file)

    # Tell index.html that sports data is loaded
    return render_template("index.html", sports_loaded=True)


# Run analysis — index.html form must point here
@sports_bp.route("/sports/participation", methods=["POST"])
def sports_participation():
    global sports_df

    if sports_df is None:
        return render_template("index.html", message="No sports dataset loaded")
    ...
    if analysis_type == "heatmap":
        plot_file = plot_heatmap(sports_df, year)
        plot_file2 = None

    elif analysis_type == "categories":
        plot_file = plot_categories(sports_df, year)
        plot_file2 = None

    elif analysis_type == "top10":
        plot_file = plot_top10(sports_df, year)
        plot_file2 = None

    elif analysis_type == "latent":
        plot_file = plot_latent(sports_df, year)
        plot_file2 = None

    elif analysis_type == "demographic":
        plot_file = plot_demographic(sports_df, year)
        plot_file2 = None

    elif analysis_type == "year_compare":
        df22 = sports_df[sports_df["Year"] == 2022]
        df23 = sports_df[sports_df["Year"] == 2023]
    ...
    # Return results into index.html
