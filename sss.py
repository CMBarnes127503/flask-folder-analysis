...
def load_sss_data(filepath):
    df = pd.read_excel(filepath, sheet_name="SSS (WW)", header=None)
...
def get_table(df, table_name):
    ...
    # NEW FIX: Skip blank / merged rows until real data begins
...
def simple_summary(df, table_name, title):
    ...
    # Special rule for Table 6 (skip "All")
...
def heatmap_plot(df, table_name, title):
    ...
    # Convert numeric columns and keep only real numeric ones
    numeric_cols = []
    ...
    # Apply real column names from your spreadsheet
    real_names = [
        "Wales",
        "RSP West Wales",
        "Pembrokeshire",
...
def sss_ww():
    ...
    if request.method == "POST":
        ...
        if "wales_file" in request.files:
            file = request.files["wales_file"]
            if file.filename:
                filepath = os.path.join(UPLOAD_FOLDER, file.filename)
                file.save(filepath)

                message = "Wales Summary Spreadsheet Loaded"
        ...
        filename = request.form.get("selected_wales_file")

        if filename:
            filepath = os.path.join(UPLOAD_FOLDER, filename)
            df = load_sss_data(filepath)
    ...
    files = os.listdir(UPLOAD_FOLDER)
    return render_template("index.html", wales_files=files)
