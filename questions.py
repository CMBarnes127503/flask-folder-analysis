...
DATA_FILE = os.path.join(UPLOAD_FOLDER, "questions_dataset.xlsx")
...
def load_dataset():
    """Load the long-format SHRN dataset safely."""
    if not os.path.exists(DATA_FILE):
        print("DEBUG: No dataset found yet.")
        return None
...
def questions():
    ...
    # ---------------------------------------------------------
    # FILE UPLOAD — persist file in session
    # ---------------------------------------------------------
    if request.method == "POST" and "questions_file" in request.files:
        file = request.files["questions_file"]

        if file.filename:
            print("DEBUG: Uploading questions_file:", file.filename)
            file.save(DATA_FILE)
            session["questions_file_loaded"] = True   # <-- persist flag

            message = "Question dataset uploaded successfully."
            df = load_dataset()

            questions_list = sorted(df["Question"].dropna().unique()) if df is not None else None
    ...
    # ---------------------------------------------------------
    # GET REQUEST — dataset persists automatically
    # ---------------------------------------------------------
    questions_list = None

    if session.get("questions_file_loaded"):
        df = load_dataset()
        if df is not None:
